...
def set_line_impedance_from_linetypes(n: Network, config: dict):
    ...
    for i, line in n.lines.iterrows():
        ...
        bus0 = float(line["bus0"])
        bus1 = float(line["bus1"])
        ...
        if (ll > 250) and v_nom == 400.0:
            ...
            #n.lines.loc[i, "s_nom"] = 2245 * 0.7 #summer
            #n.lines.loc[i, "s_nom_2"] = 2245 #summer

            n.lines.loc[i, "s_nom"] = 3382 * 0.7 #winter
            n.lines.loc[i, "s_nom_2"] = 3382 #winter
            ...
            #below is the seriescompensation. Comment out if not in use. There is also one line in the next "elfi" statement
            if bus0 == 6630 and bus1 == 6844:
                n.lines.loc[i, "x_pu"] = x * 0.6 / Z_base

            elif bus0 == 6630 and bus1 == 6828:
                n.lines.loc[i, "x_pu"] = x * 0.6 / Z_base

            elif bus0 == 6637 and bus1 == 6828:
                n.lines.loc[i, "x_pu"] = x * 0.6 / Z_base

            elif bus0 == 6639 and bus1 == 6830:
                n.lines.loc[i, "x_pu"] = x * 0.6 / Z_base

            elif bus0 == 6631 and bus1 == 6796:
                n.lines.loc[i, "x_pu"] = x * 0.6 / Z_base

            elif bus0 == 6581 and bus1 == 6802:
                n.lines.loc[i, "x_pu"] = x * 0.6 / Z_base

            elif bus0 == 6583 and bus1 == 6803:
                n.lines.loc[i, "x_pu"] = x * 0.6 / Z_base
            ...
            n.lines.loc[i, "s_nom"] = 3382 * 0.7
            n.lines.loc[i, "s_nom_2"] = 3382
            b_per_length = (  # [S/km]
                    2 * np.pi * 50
                    * 13.22 * 1e-9
                    * num_parallel
            ...
            #n.lines.loc[i, "s_nom"] = 450 * 2 * 0.7 #summer
            #n.lines.loc[i, "s_nom_2"] = 450 * 2 #summer

            n.lines.loc[i, "s_nom"] = 671 * 2 * 0.7 #winter
            n.lines.loc[i, "s_nom_2"] = 671 * 2 #winter
            ...
            n.lines.loc[i, "s_nom"] = 746 * 0.7 #summer
            n.lines.loc[i, "s_nom_2"] = 746 #summer

            n.lines.loc[i, "s_nom"] = 1117 * 0.7 #winter
            n.lines.loc[i, "s_nom_2"] = 1117 #winter
...
#redistributes load from low voltage side of transformer to the high voltage side, not necessairy when running with regional distribution of load
...
def set_link_p_set(n: Network, cross_border_flow: pd.DataFrame):
    ...
    # SydVästlänken link is not commissioned before ~2022
    # Change to False to include it in the model
    n.links.loc['14806', "under_construction"] = True
    n.links.loc['14806', "p_nom"] = 1200
    n.links.loc['14806', 'p_set'] = 0


def scale_generation_per_bidding_zone(n: Network, entsoe_generation: pd.DataFrame, scaled_generators):

    #add generators to remove here
    ...
    all_generation_units = pd.concat([all_generation_units, scaled_generators])
    n.generators = pd.concat([n.generators, scaled_generators])
...
# this uses the pypsa way of distributing the loads, initially commented below, instead the per region distribution is used defined in def scale_loads_per_region
def scale_load_per_bidding_zone(n: Network, load_per_bidding_zone: pd.Series, loads_rescaling):
...
#scales loads per region
def scale_load_per_region(n: Network, load_per_bidding_zone: pd.Series, loads_rescaling):
    n.loads["p_set_country"] = n.loads["p_set"]  # save for comparison
    loads_rescaling["bus"] = loads_rescaling["bus"].astype(str)

    # Create a new DataFrame with the additional buses from loads_rescaling
    new_loads = pd.concat([n.loads, loads_rescaling], ignore_index=True, sort=False)

    # Update the existing n.loads DataFrame with the merged data
    n.loads = new_loads.copy()

    # Set the load_factor and bidding_zone columns based on the merged values
    n.loads["load_factor"] = n.loads["load_factor"].fillna(0)  # Fill missing load_factor values with 0
    n.loads["bidding_zone"] = n.loads["bidding_zone"].fillna("")  # Fill missing bidding_zone values with empty string

    # Multiplying bidding zone load with load_factor
    n.loads["p_set"] = n.loads.groupby("bidding_zone", group_keys=False)["load_factor"].apply(
        lambda x: x * (load_per_bidding_zone[x.name] if (x.name in load_per_bidding_zone) else 0)
    )
...
def add_manual_lines(n, manual_lines):
...
if __name__ == "__main__":
    ...
    add_manual_lines(n, manual_lines)
    ...
    scaled_generators = pd.read_excel(snakemake.input.scaled_generators, index_col=0)
    scale_generation_per_bidding_zone(n, entsoe_generation, scaled_generators)

    logger.info('Removing generation and load outside Nordic area')
    remove_load_generation_outside_sync_area(n)

    logger.info('Scaling load per region')
    entsoe_load = pd.read_csv(snakemake.input.load, index_col=0)['load']
    loads_rescaling = pd.read_excel(snakemake.input.loads_rescaling)
    scale_load_per_region(n, entsoe_load, loads_rescaling)

    #the pypsa way of distributing load (60% gdp - 40% population)
    #logger.info('Scaling load per bidding zone')
    #entsoe_load = pd.read_csv(snakemake.input.load, index_col=0)['load']
    #loads_rescaling = pd.read_excel(snakemake.input.loads_rescaling)
    #scale_load_per_bidding_zone(n, entsoe_load, loads_rescaling)
